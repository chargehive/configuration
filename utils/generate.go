...
const (
	...
	confConnAuthorizeNet  Template = "con_authorizenet"
...
var Templates = map[Template]string{
	...
	confConnAuthorizeNet:  "Connector: Authorize.net",
...
func buildSpec(conf Template) (object.Specification, error) {
	...
	case confConnAuthorizeNet:
		j, _ := json.Marshal(connectorconfig.AuthorizeCredentials{})
		return connector.Connector{Library: string(connectorconfig.LibraryAuthorizeNet), Configuration: j}, nil
	...
	case confConnWorldPay:
		j, _ := json.Marshal(connectorconfig.WorldpayCredentials{
			...
			CardinalApiKey:        &chg,
			CardinalOrgUnitId:     &chg,
			ApplePay: &connectorconfig.ApplePayCredentials{
	...
	case confPolSCA:
		return policy.ScaPolicy{ShouldIdentify: new(bool), ShouldChallengeOptional: new(bool), ShouldByPassChallenge: "cascade", ShouldAuthOnError: new(bool), ShouldAuthOnN: new(bool), ShouldAuthOnR: new(bool), ShouldAuthOnU: new(bool), RequireSca: new(bool)}, nil
